...
```{r Planning}
  ...
  ## 2.1 Word count and tokenization for business key activity extraction, then filter for fishing related only
  ## 2.2 Prepare fishing related node and edge for further processing
  ## 2.3. Transform the business relationship by counting company to company, company to owner, company to contact information etc,  categorize relationship manually, give some labels
...
```{r selectedMaritimeCompanies}
 ...
 desired_words <- c("fish", "seafood", "frozen", "fresh", "salmon", "canned", "meat", "tuna", "shrimp", "shellfish", "sea", "squid", "seafoods", "marine", "fillets", "foods")
  ...
  # MC3_edges_fishing <- MC3_edges_clean_Join %>%
  # filter(
  #   source %in% MC3_nodes$id[grep("fish|seafood|frozen|fresh|salmon|canned|tuna|shrimp|shellfish|sea|squid|fishing|seafoods|marine|fillets", MC3_nodes$product_services, ignore.case = TRUE)]) %>%
  # distinct()
...
Next,  extract all fish active related nodes to construct a graph:
```{r deriveNewNodeData}
...
<!-- # ```{r FAILED:fillInTargetType}
  #Find where are the rows without type info in current fishNetwork Nodes
  empty_type_rows <- MC3_nodes_fishNetwork %>%
  filter(is.na(type))

  #try to match the empty rows with target edge type information
  ...
  MC3_nodes_fishNetwork2 <- MC3_nodes_fishNetwork %>%
...
## 3.Consolidate counting information
...
Next, manually add some label for anomalies.
...
```{r abnormalitiesForTooManyOwnersSingleCompany}
  ...
  visGraph <- visNetwork(MC3_nodes_toomanyownersView,MC3_edges_toomanyowners)%>%
...
##USE THIS LATER?? **Handling missing values:** for some or the product/services, there's blank value such as "character(0)", **recode these value to NA these value before pass them for text sensing**.:
